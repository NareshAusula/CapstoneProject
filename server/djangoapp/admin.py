...
class CarModelAdmin(admin.ModelAdmin):
    list_display = (
        ...
        "type",
        "year",
    )  # Fields shown in the list view (e.g., "Camry | Toyota | Sedan | 2023").
    list_filter = (
        "car_make",
        "type",
        "year",
    )  # Adds filters on the right (filter by make, type, or year).
    search_fields = (
        "name",
        "car_make__name",
    )  # Enables search by model name or make name (car_make__name).
